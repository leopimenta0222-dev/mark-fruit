...
export default function OrderDetail() {
  ...
  if (loading) return <div className="min-h-[60vh] grid place-items-center text-stone-500 dark:text-stone-400">Carregando pedido...</div>;
    ...
    return (
      <div className="min-h-[60vh] bg-stone-50 dark:bg-stone-950 px-4 grid place-items-center text-center">
        <div className="card max-w-lg p-8">
          ...
          <p className="mt-2 text-stone-500 dark:text-stone-400">Ele pode não existir ou não pertencer à sua conta.</p>
  ...
  return (
    <div className="min-h-[65vh] bg-stone-50 dark:bg-stone-950 px-4 py-7 pb-24 md:py-10">
      <div className="mx-auto max-w-5xl">
        <Link to={backPath} className="inline-flex items-center gap-2 text-sm font-bold text-stone-600 dark:text-stone-300 hover:text-brand-700 dark:hover:text-brand-300"><ArrowLeft size={17} /> Voltar</Link>
        <header className="mt-5 mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-stone-500 dark:text-stone-400">Criado em {formatDate(order.createdAt)}</p>
          </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem] items-start">
          <div className="space-y-5">
            <section className="card p-5 md:p-6">
              ...
              <p className="mt-4 text-sm text-stone-500 dark:text-stone-400">{ORDER_STATUS[order.status]?.label}. As alterações são salvas no Supabase e aparecem para comprador e produtor.</p>
            </section>
              ...
              <div className="mt-4 divide-y divide-stone-100 dark:divide-stone-800">
                {order.items.map((item) => (
                  <div key={item.id} className="py-4 first:pt-0 last:pb-0 flex justify-between gap-4">
                    <div><strong className="block">{item.title}</strong><span className="text-sm text-stone-500 dark:text-stone-400">{item.quantity} × {formatCurrency(item.unitPrice)}</span></div>
                    <strong>{formatCurrency(item.subtotal)}</strong>
                  </div>
                ))}
              </div>
              <div className="mt-5 pt-4 border-t-2 border-stone-200 dark:border-stone-700 flex items-end justify-between"><span className="font-bold">Total</span><strong className="text-2xl">{formatCurrency(order.total)}</strong></div>
            </section>
          </div>
          ...
          <aside className="space-y-4">
            <section className="card p-5">
              ...
              <p className="mt-3 text-xs font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">{producerView ? "Consumidor" : "Produtor"}</p>
              <h2 className="mt-1 text-lg font-extrabold">{contact?.name || "Contato"}</h2>
              {(contact?.city || contact?.state) && <p className="mt-2 text-sm text-stone-500 dark:text-stone-400">{[contact.city, contact.state].filter(Boolean).join(" · ")}</p>}
            ...
            <section className="card p-5">
              ...
              <p className="mt-2 text-sm text-stone-500 dark:text-stone-400">{order.fulfillmentMethod === "DELIVERY" ? addressLabel(order.deliveryAddress) : "Local e horário combinados diretamente com o produtor."}</p>
            </section>

            <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-950 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-100">
              <CreditCard size={20} />
