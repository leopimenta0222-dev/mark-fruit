  ...
  return (
    <article className="card p-5 md:p-6">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          {(person?.city || person?.state) && (
            <p className="mt-1 flex items-center gap-1.5 text-sm text-stone-500 dark:text-stone-400"><MapPin size={15} /> {[person.city, person.state].filter(Boolean).join(" · ")}</p>
          )}
        </div>
      ...
      <div className="mt-5 border-y border-stone-100 dark:border-stone-800 py-4 space-y-2">
        ...
        {order.items.length > 2 && <p className="text-xs text-stone-500 dark:text-stone-400">+ {order.items.length - 2} outro(s) item(ns)</p>}
      </div>
