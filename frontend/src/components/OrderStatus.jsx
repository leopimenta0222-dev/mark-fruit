  ...
  return (
    <div aria-label={`Status: ${ORDER_STATUS[status]?.label || status}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        {!compact && <span className="text-xs text-stone-500 dark:text-stone-400">Etapa {currentIndex + 1} de {STEPS.length}</span>}
      </div>
      {!compact && (
        <div className="mt-3 flex" aria-hidden="true">
          {STEPS.map((step, index) => (
            <div key={step} className="flex flex-1 items-center last:flex-none">
              <span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full border-2 ${index <= currentIndex ? "border-brand-600 bg-brand-600 text-white" : "border-stone-300 bg-white text-transparent dark:border-stone-600 dark:bg-stone-900"}`}>
